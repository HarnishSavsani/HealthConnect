...
def setup_healthcare():
	if healthconnect.db.exists("Medical Department", "Cardiology"):
		# already setup
		return
...
def setup_domain():
	...
	"""
	domain = healthconnect.get_doc("Domain", "Healthcare")
	domain.setup_domain()

	# update active domains
	if "Healthcare" not in healthconnect.get_active_domains():
		has_domain = healthconnect.get_doc(
			{
				"doctype": "Has Domain",
...
def create_default_root_service_units():
	...
	companies = healthconnect.get_all("Company")
...
def create_medical_departments():
	...
	for department in departments:
		mediacal_department = healthconnect.new_doc("Medical Department")
		mediacal_department.department = _(department)
		try:
			mediacal_department.save()
		except healthconnect.DuplicateEntryError:
			pass
...
def create_antibiotics():
	...
	for a in abt:
		antibiotic = healthconnect.new_doc("Antibiotic")
...
def create_healthcare_item_groups():
	...
	if not healthconnect.db.exists(item_group["doctype"], item_group["item_group_name"]):
...
def delete_custom_records():
	...
	for record in records:
		healthconnect.db.delete(record.get("doctype"), record.get("name"))

	healthconnect.db.set_single_value("Portal Settings", "default_role", "")


def remove_from_active_domains():
	"""Remove Healthcare from active domains in Domain Settings"""
	healthconnect.db.delete("Has Domain", {"domain": "Healthcare"})


def remove_portal_settings_menu_items():
	"""Remove menu items added in Portal Settings"""
	menu_items = healthconnect.get_hooks("standard_portal_menu_items", app_name="healthcare")
	for item in menu_items:
		healthconnect.db.delete("Portal Menu Item", item)
