# Copyright (c) 2013, healthconnect Technologies Pvt. Ltd. and contributors
# For license information, please see license.txt

import healthconnect
from erpnext.accounts.utils import get_fiscal_year
from healthconnect import _, scrub
from healthconnect.database.query import OPERATOR_MAP
from healthconnect.utils import add_days, add_to_date, flt, getdate
...
class DiagnosisTrends(object):
	...
	def __init__(self, filters=None):
		...
		self.filters = healthconnect._dict(filters or {})
		self.months = [
			"Jan",
			"Feb",
			"Mar",
	...
	def get_data(self):
		pe_diagnosis = healthconnect.qb.DocType("Patient Encounter Diagnosis")
		query = (
			healthconnect.qb.from_(pe_diagnosis)
		...
		if department:
			encounters = healthconnect.get_all(
	...
	def get_periodic_data(self):
		self.appointment_periodic_data = healthconnect._dict()

		for d in self.entries:
			period = self.get_period(d.get("creation"))
			self.appointment_periodic_data.setdefault(d.diagnosis, healthconnect._dict()).setdefault(period, 0.0)
