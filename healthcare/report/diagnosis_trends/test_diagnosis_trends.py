...
from healthconnect.utils import add_days, add_months, getdate
...
class TestDiagnosisTrends(healthconnectTestCase):
	@classmethod
	def setUpClass(cls):
	...
	def create_diagnosis(cls):
		medical_department = healthconnect.get_doc(
		...
		patient = healthconnect.get_list("Patient")[0]
		...
		encounter = healthconnect.get_list(
			"Patient Encounter",
		)[0]

		try:
			cls.diagnosis = healthconnect.get_doc(
				{
					"doctype": "Diagnosis",
					"diagnosis": "Fever",
				}
		...
		try:
			cls.diagnosis_cardio = healthconnect.get_doc(
				{
					"doctype": "Diagnosis",
		...
		encounter = healthconnect.get_doc("Patient Encounter", encounter["name"])
		encounter.append(
			"diagnosis",
			{
				"diagnosis": "Fever",
	...
	def test_report_data_with_filters(self):
		medical_department = healthconnect.get_doc("Medical Department", "Cardiology")
