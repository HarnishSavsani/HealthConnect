...
healthconnect.provide('erpnext.queries');
healthconnect.ui.form.on('Patient Appointment', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.patient) {
			frm.add_custom_button(__('Patient History'), function() {
				healthconnect.route_options = { 'patient': frm.doc.patient };
		...
		if (["Open", "Checked In"].includes(frm.doc.status) || (frm.doc.status == 'Scheduled' && !frm.doc.__islocal)) {
			...
			if (frm.doc.procedure_template) {
				frm.add_custom_button(__('Clinical Procedure'), function() {
					healthconnect.model.open_mapped_doc({
	...
	appointment_type: function(frm) {
		if (frm.doc.appointment_type) {
			if (frm.doc.appointment_for && frm.doc[healthconnect.scrub(frm.doc.appointment_for)]) {
				frm.events.set_payment_details(frm);
			}
		}
	},
	...
	set_payment_details: function(frm) {
		healthconnect.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing').then(val => {
			if (val) {
				healthconnect.call({
					...
					callback: function(data) {
						if (data.message) {
							healthconnect.model.set_value(frm.doctype, frm.docname, 'paid_amount', data.message.practitioner_charge);
							healthconnect.model.set_value(frm.doctype, frm.docname, 'billing_item', data.message.service_item);
						}
					}
				});
			}
	...
	therapy_type: function(frm) {
		if (frm.doc.therapy_type) {
			healthconnect.db.get_value('Therapy Type', frm.doc.therapy_type, 'default_duration', (r) => {
...
let check_and_set_availability = function(frm) {
	...
	function show_availability() {
		...
		let d = new healthconnect.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: 'Column Break' },
				{ fieldtype: 'Date', reqd: 1, fieldname: 'appointment_date', label: 'Date', min_date: new Date(healthconnect.datetime.get_today()) },
	...
	function validate_fee_validity(frm, d) {
		...
		if (d.get_value('appointment_date') && !frm.doc.invoiced) {
			healthconnect.db.get_single_value('Healthcare Settings', 'enable_free_follow_ups').then(async function (val) {
				if (val) {
					fee_validity = (await healthconnect.call(
					...
					if (!fee_validity) {
						...
						let payment_details = (await healthconnect.call(
...
let show_therapy_types = function(frm, result) {
	var d = new healthconnect.ui.Dialog({
...
let create_vital_signs = function(frm) {
	if (!frm.doc.patient) {
		healthconnect.throw(__('Please select patient'));
	}
	...
	healthconnect.new_doc('Vital Signs');
};

let update_status = function(frm, status) {
	let doc = frm.doc;
	healthconnect.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			healthconnect.call({
