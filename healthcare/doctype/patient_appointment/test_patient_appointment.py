...
from healthconnect.utils import add_days, flt, get_time, getdate, now_datetime, nowdate
...
class TestPatientAppointment(healthconnectTestCase):
	def setUp(self):
		healthconnect.db.sql("""delete from `tabPatient Appointment`""")
		healthconnect.db.sql("""delete from `tabFee Validity`""")
		healthconnect.db.sql("""delete from `tabPatient Encounter`""")
		make_pos_profile()
		healthconnect.db.sql("""delete from `tabHealthcare Service Unit` where name like '_Test %'""")
	...
	def test_status(self):
		...
		healthconnect.db.set_single_value("Healthcare Settings", "automate_appointment_invoicing", 0)
		...
		self.assertEqual(
			healthconnect.db.get_value("Patient Appointment", appointment.name, "status"), "Closed"
		)
		encounter.cancel()
		self.assertEqual(healthconnect.db.get_value("Patient Appointment", appointment.name, "status"), "Open")

	def test_start_encounter(self):
		patient, practitioner = create_healthcare_docs()
		healthconnect.db.set_single_value("Healthcare Settings", "automate_appointment_invoicing", 1)
		...
		self.assertEqual(
			encounter.invoiced, healthconnect.db.get_value("Patient Appointment", appointment.name, "invoiced")
		)

	def test_auto_invoicing(self):
		patient, practitioner = create_healthcare_docs()
		healthconnect.db.set_single_value("Healthcare Settings", "enable_free_follow_ups", 0)
		healthconnect.db.set_single_value("Healthcare Settings", "automate_appointment_invoicing", 0)
		appointment = create_appointment(patient, practitioner, nowdate())
		self.assertEqual(healthconnect.db.get_value("Patient Appointment", appointment.name, "invoiced"), 0)

		healthconnect.db.set_single_value("Healthcare Settings", "automate_appointment_invoicing", 1)
		appointment = create_appointment(patient, practitioner, add_days(nowdate(), 2), invoice=1)
		self.assertEqual(healthconnect.db.get_value("Patient Appointment", appointment.name, "invoiced"), 1)
		sales_invoice_name = healthconnect.db.get_value(
			"Sales Invoice Item", {"reference_dn": appointment.name}, "parent"
		)
		self.assertTrue(sales_invoice_name)
		self.assertEqual(
			healthconnect.db.get_value("Sales Invoice", sales_invoice_name, "company"), appointment.company
		)
		self.assertEqual(
			healthconnect.db.get_value("Sales Invoice", sales_invoice_name, "patient"), appointment.patient
		)
		self.assertEqual(
			healthconnect.db.get_value("Sales Invoice", sales_invoice_name, "paid_amount"), appointment.paid_amount
		)
	...
	def test_auto_invoicing_based_on_department(self):
		healthconnect.db.set_single_value("Healthcare Settings", "enable_free_follow_ups", 1)
		healthconnect.db.set_single_value("Healthcare Settings", "automate_appointment_invoicing", 1)
		...
		appointment = healthconnect.new_doc("Patient Appointment")
	...
	def test_appointment_cancel(self):
		...
		healthconnect.db.set_single_value("Healthcare Settings", "enable_free_follow_ups", 1)
		appointment = create_appointment(patient, practitioner, nowdate())
		fee_validity = healthconnect.db.get_value(
		...
		self.assertEqual(healthconnect.db.get_value("Fee Validity", fee_validity, "visited"), 0)
		...
		self.assertEqual(healthconnect.db.get_value("Sales Invoice", sales_invoice_name, "status"), "Cancelled")
	...
	def test_invalid_healthcare_service_unit_validation(self):
		...
		self.assertRaises(healthconnect.exceptions.ValidationError, appointment.save)
	...
	def test_payment_should_be_mandatory_for_new_patient_appointment(self):
		healthconnect.db.set_single_value("Healthcare Settings", "enable_free_follow_ups", 1)
		healthconnect.db.set_single_value("Healthcare Settings", "automate_appointment_invoicing", 1)
		healthconnect.db.set_single_value("Healthcare Settings", "max_visits", 3)
		healthconnect.db.set_single_value("Healthcare Settings", "valid_days", 30)
	...
	def test_sales_invoice_should_be_generated_for_new_patient_appointment(self):
		patient, practitioner = create_healthcare_docs()
		healthconnect.db.set_single_value("Healthcare Settings", "automate_appointment_invoicing", 1)
		invoice_count = healthconnect.db.count("Sales Invoice")

		assert check_is_new_patient(patient)
		create_appointment(patient, practitioner, nowdate())
		new_invoice_count = healthconnect.db.count("Sales Invoice")
	...
	def test_patient_appointment_should_consider_permissions_while_fetching_appointments(self):
		...
		new_practitioner = healthconnect.get_doc("Healthcare Practitioner", new_practitioner)
		new_practitioner.user_id = user.email
		new_practitioner.save()

		healthconnect.set_user(user.name)
		appointments = healthconnect.get_list("Patient Appointment")
		assert len(appointments) == 1

		healthconnect.set_user("Administrator")
		appointments = healthconnect.get_list("Patient Appointment")
...
def create_patient(
	...
	if healthconnect.db.exists("Patient", {"firstname": f"_Test Patient {str(id)}"}):
		patient = healthconnect.db.get_value("Patient", {"first_name": f"_Test Patient {str(id)}"}, ["name"])
		return patient
...
def create_medical_department(id=0):
	if healthconnect.db.exists("Medical Department", f"_Test Medical Department {str(id)}"):
		return f"_Test Medical Department {str(id)}"

	medical_department = healthconnect.new_doc("Medical Department")
...
def create_practitioner(id=0, medical_department=None):
	if healthconnect.db.exists(
		...
		practitioner = healthconnect.db.get_value(
			"Healthcare Practitioner", {"firstname": f"_Test Healthcare Practitioner {str(id)}"}, ["name"]
		)
		return practitioner

	practitioner = healthconnect.new_doc("Healthcare Practitioner")
...
def create_appointment(
	...
	item = create_healthcare_service_items()
	healthconnect.db.set_single_value("Healthcare Settings", "inpatient_visit_charge_item", item)
	healthconnect.db.set_single_value("Healthcare Settings", "op_consulting_charge_item", item)
	appointment = healthconnect.new_doc("Patient Appointment")
...
def create_healthcare_service_items():
	if healthconnect.db.exists("Item", "HLC-SI-001"):
		return "HLC-SI-001"

	item = healthconnect.new_doc("Item")
...
def create_clinical_procedure_template():
	if healthconnect.db.exists("Clinical Procedure Template", "Knee Surgery and Rehab"):
		return healthconnect.get_doc("Clinical Procedure Template", "Knee Surgery and Rehab")

	template = healthconnect.new_doc("Clinical Procedure Template")
...
def create_appointment_type(args=None):  # nosemgrep
	if not args:
		args = healthconnect.local.form_dict

	name = args.get("name", "_Test Appointment Type")

	if healthconnect.db.exists("Appointment Type", name):
		return healthconnect.get_doc("Appointment Type", name)
		...
		return healthconnect.get_doc(
			{
				...
				"price_list": args.get("price_list") or healthconnect.db.get_value("Price List", {"selling": 1}),
...
def create_user(email=None, roles=None):
	if not email:
		email = "{}@healthconnect.com".format(healthconnect.utils.random_string(10))
	user = healthconnect.db.exists("User", email)
	if not user:
		user = healthconnect.get_doc(
			{
				"doctype": "User",
				"email": email,
...
def create_service_unit_type(id=0, allow_appointments=1, overlap_appointments=0):
	if healthconnect.db.exists("Healthcare Service Unit Type", f"_Test Service Unit Type {str(id)}"):
...
def create_service_unit(id=0, service_unit_type=None, service_unit_capacity=0):
	if healthconnect.db.exists("Healthcare Service Unit", f"_Test Service Unit {str(id)}"):
...
def test_appointment_reschedule(self, appointment):
	...
	appointment.save()
	self.assertTrue(
		healthconnect.db.exists("Event", {"name": appointment.event, "starts_on": new_appointment_datetime})
	)


def test_appointment_cancel(self, appointment):
	update_status(appointment.name, "Cancelled")
	self.assertTrue(healthconnect.db.exists("Event", {"name": appointment.event, "status": "Cancelled"}))
