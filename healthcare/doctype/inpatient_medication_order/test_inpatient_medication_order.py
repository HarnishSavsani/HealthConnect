...
from healthconnect.utils import add_days, getdate, now_datetime
...
class TestInpatientMedicationOrder(healthconnectTestCase):
	def setUp(self):
	...
	def test_order_creation(self):
		...
		prescription_dosage = healthconnect.get_doc("Prescription Dosage", "1-1-1")
	...
	def test_inpatient_validation(self):
		# Discharge
		schedule_discharge(healthconnect.as_json({"patient": self.patient}))
		...
		self.assertRaises(healthconnect.ValidationError, ipmo.insert)
	...
	def test_status(self):
		...
		filters = healthconnect._dict(from_date=getdate(), to_date=getdate(), from_time="", to_time="")
		ipme = create_ipme(filters)
		ipme.submit()
		ipmo.reload()
	...
	def tearDown(self):
		if healthconnect.db.get_value("Patient", self.patient, "inpatient_record"):
			# cleanup - Discharge
			schedule_discharge(healthconnect.as_json({"patient": self.patient}))
			self.ip_record.reload()
			mark_invoiced_inpatient_occupancy(self.ip_record)
		...
		for doctype in ["Inpatient Medication Entry", "Inpatient Medication Order"]:
			healthconnect.db.sql("delete from `tab{doctype}`".format(doctype=doctype))


def create_dosage_form():
	if not healthconnect.db.exists("Dosage Form", "Tablet"):
		healthconnect.get_doc({"doctype": "Dosage Form", "dosage_form": "Tablet"}).insert()


def create_drug(item=None):
	if not item:
		item = "Dextromethorphan"
	drug = healthconnect.db.exists("Item", {"item_code": "Dextromethorphan"})
	if not drug:
		drug = healthconnect.get_doc(
			{
				"doctype": "Item",
...
def create_ipmo(patient):
	orders = get_orders()
	ipmo = healthconnect.new_doc("Inpatient Medication Order")
...
def create_ipme(filters, update_stock=0):
	ipme = healthconnect.new_doc("Inpatient Medication Entry")
