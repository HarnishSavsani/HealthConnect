...
healthconnect.ui.form.on('Healthcare Practitioner', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		healthconnect.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Healthcare Practitioner'};

		if (!frm.is_new()) {
			healthconnect.contacts.render_address_and_contact(frm);
		} else {
			healthconnect.contacts.clear_address_and_contact(frm);
		}
	...
	practitioner_primary_address: function(frm) {
		if (frm.doc.practitioner_primary_address) {
			healthconnect.call({
				method: 'healthconnect.contacts.doctype.address.address.get_address_display',
				args: {
	...
	user_id: function(frm) {
		if (frm.doc.user_id && frm.doc.practitioner_type == 'Internal') {
			healthconnect.call({
				'method': 'healthconnect.client.get',
				args: {
					doctype: 'User',
					name: frm.doc.user_id
				},
				callback: function (data) {
					healthconnect.model.get_value('Employee', {'user_id': frm.doc.user_id}, 'name',
						function(data) {
							if (data) {
								if (!frm.doc.employee || frm.doc.employee != data.name)
									healthconnect.model.set_value(frm.doctype, frm.docname, 'employee', data.name);
							} else {
								healthconnect.model.set_value(frm.doctype, frm.docname, 'employee', '');
							}
						}
					);

					if (!frm.doc.first_name || frm.doc.first_name != data.message.first_name)
						healthconnect.model.set_value(frm.doctype,frm.docname, 'first_name', data.message.first_name);
					if (!frm.doc.middle_name || frm.doc.middle_name != data.message.middle_name)
						healthconnect.model.set_value(frm.doctype,frm.docname, 'middle_name', data.message.middle_name);
					if (!frm.doc.last_name || frm.doc.last_name != data.message.last_name)
						healthconnect.model.set_value(frm.doctype,frm.docname, 'last_name', data.message.last_name);
					if (!frm.doc.mobile_phone || frm.doc.mobile_phone != data.message.mobile_no)
						healthconnect.model.set_value(frm.doctype,frm.docname, 'mobile_phone', data.message.mobile_no);
				}
			});
			...
			healthconnect.call({
				...
				callback: function (data) {
					...
					if (!frm.doc.designation || frm.doc.designation != data.message.designation)
						healthconnect.model.set_value(frm.doctype,frm.docname, 'designation', data.message.designation);
					if (!frm.doc.first_name || !frm.doc.user_id){
						healthconnect.model.set_value(frm.doctype,frm.docname, 'first_name', data.message.first_name);
						healthconnect.model.set_value(frm.doctype,frm.docname, 'middle_name', '');
						healthconnect.model.set_value(frm.doctype,frm.docname, 'last_name', data.message.last_name);
					}
					if (!frm.doc.mobile_phone || !frm.doc.user_id)
						healthconnect.model.set_value(frm.doctype,frm.docname, 'mobile_phone', data.message.cell_number);
					if (!frm.doc.address || frm.doc.address != data.message.current_address)
						healthconnect.model.set_value(frm.doctype,frm.docname, 'address', data.message.current_address);
				}
			});
		}
	}
...
healthconnect.tour['Healthcare Practitioner'] = [
	{
		fieldname: 'employee',
		title: __('Employee'),
