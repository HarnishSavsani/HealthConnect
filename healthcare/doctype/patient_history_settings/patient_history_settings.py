...
from healthconnect.utils import cint, cstr
...
class PatientHistorySettings(Document):
	...
	def validate_submittable_doctypes(self):
		for entry in self.custom_doctypes:
			if not cint(healthconnect.db.get_value("DocType", entry.document_type, "is_submittable")):
				msg = _("Row #{0}: Document Type {1} is not submittable.").format(
					entry.idx, healthconnect.bold(entry.document_type)
				)
				msg += _("Patient Medical Record can only be created for submittable document types.")
				healthconnect.throw(msg)

	def validate_date_fieldnames(self):
		for entry in self.custom_doctypes:
			field = healthconnect.get_meta(entry.document_type).get_field(entry.date_fieldname)
			if not field:
				healthconnect.throw(
					_("Row #{0}: No such Field named {1} found in the Document Type {2}.").format(
						entry.idx, healthconnect.bold(entry.date_fieldname), healthconnect.bold(entry.document_type)
					)
				)
	...
	def get_doctype_fields(self, document_type, fields):
		multicheck_fields = []
		doc_fields = healthconnect.get_meta(document_type).fields

		for field in doc_fields:
			if (
				field.fieldtype not in healthconnect.model.no_value_fields
				or field.fieldtype in healthconnect.model.table_fields
	...
	def get_date_field_for_dt(self, document_type):
		meta = healthconnect.get_meta(document_type)
...
def create_medical_record(doc, method=None):
	...
	if healthconnect.db.exists("Patient Medical Record", {"reference_name": doc.name}):
		return

	subject = set_subject_field(doc)
	date_field = get_date_field(doc.doctype)
	medical_record = healthconnect.new_doc("Patient Medical Record")
...
def update_medical_record(doc, method=None):
	...
	medical_record_id = healthconnect.db.exists("Patient Medical Record", {"reference_name": doc.name})

	if medical_record_id:
		subject = set_subject_field(doc)
		healthconnect.db.set_value("Patient Medical Record", medical_record_id[0][0], "subject", subject)
	else:
...
def delete_medical_record(doc, method=None):
	...
	record = healthconnect.db.exists("Patient Medical Record", {"reference_name": doc.name})
	if record:
		healthconnect.delete_doc("Patient Medical Record", record, force=1)


def set_subject_field(doc):
	from healthconnect.utils.formatters import format_value

	meta = healthconnect.get_meta(doc.doctype)
	subject = ""
	...
	for entry in patient_history_fields:
		...
		if entry.get("fieldtype") == "Table" and doc.get(fieldname):
			...
			subject += healthconnect.bold(_(entry.get("label")) + ":") + "<br>" + cstr(formatted_value) + "<br>"

		else:
			if doc.get(fieldname):
				formatted_value = format_value(doc.get(fieldname), meta.get_field(fieldname), doc)
				subject += healthconnect.bold(_(entry.get("label")) + ":") + cstr(formatted_value) + "<br>"
...
def get_date_field(doctype):
	...
	return healthconnect.db.get_value(dt, {"document_type": doctype}, "date_fieldname")


def get_patient_history_fields(doc):
	dt = get_patient_history_config_dt(doc.doctype)
	patient_history_fields = healthconnect.db.get_value(
...
def get_patient_history_config_dt(doctype):
	if healthconnect.db.get_value("DocType", doctype, "custom"):
...
def validate_medical_record_required(doc):
	if (
		healthconnect.flags.in_patch
		or healthconnect.flags.in_install
		or healthconnect.flags.in_setup_wizard
...
def get_module(doc):
	...
	if not module:
		module = healthconnect.db.get_value("DocType", doc.doctype, "module")
