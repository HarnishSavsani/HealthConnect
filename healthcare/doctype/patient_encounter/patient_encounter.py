...
from healthconnect.utils import add_days, getdate
...
class PatientEncounter(Document):
	...
	def on_cancel(self):
		if self.appointment:
			healthconnect.db.set_value("Patient Appointment", self.appointment, "status", "Open")
	...
	def get_applicable_treatment_plans(encounter):
		...
		if diagnosis:
			...
			diagnosis = healthconnect.db.get_all("Patient Encounter Diagnosis", filters=filters, fields="*")
		...
		if symptoms:
			...
			symptoms = healthconnect.db.get_all("Patient Encounter Symptom", filters=filters, fields="*")
		...
		plans = healthconnect.get_list("Treatment Plan Template", fields="*", filters=plan_filters)
	...
	def set_treatment_plan(self, plan):
		plan_doc = healthconnect.get_doc("Treatment Plan Template", plan)

		for plan_item in plan_doc.items:
			self.set_treatment_plan_item(plan_item)

		for drug in plan_doc.drugs:
			self.append("drug_prescription", (healthconnect.copy_doc(drug)).as_dict())
...
def make_ip_medication_order(source_name, target_doc=None):
	def set_missing_values(source, target):
		...
		for entry in source.drug_prescription:
			if entry.drug_code:
				dosage = healthconnect.get_doc("Prescription Dosage", entry.dosage)
...
def get_prescription_dates(period, start_date):
	prescription_duration = healthconnect.get_doc("Prescription Duration", period)
...
def create_therapy_plan(encounter):
	if len(encounter.therapies):
		doc = healthconnect.new_doc("Therapy Plan")
		...
		if doc.get("name"):
			...
			healthconnect.msgprint(
				_("Therapy Plan {0} created successfully.").format(healthconnect.bold(doc.name)), alert=True
			)


def delete_ip_medication_order(encounter):
	record = healthconnect.db.exists("Inpatient Medication Order", {"patient_encounter": encounter.name})
	if record:
		healthconnect.delete_doc("Inpatient Medication Order", record, force=1)
