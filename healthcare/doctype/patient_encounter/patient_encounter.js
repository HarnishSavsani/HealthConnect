...
healthconnect.ui.form.on('Patient Encounter', {
	onload: function(frm) {
		if (!frm.doc.__islocal && frm.doc.docstatus === 1 &&
			frm.doc.inpatient_status == 'Admission Scheduled') {
				healthconnect.db.get_value('Inpatient Record', frm.doc.inpatient_record,
	...
	refresh: function(frm) {
		...
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Patient History'), function() {
				if (frm.doc.patient) {
					...
					healthconnect.msgprint(__('Please select Patient'));
				}
			},__('View'));
		...
		frm.set_query("medical_code", "codification_table", function(doc, cdt, cdn) {
			let row = healthconnect.get_doc(cdt, cdn);
		...
		if (frm.doc.google_meet_link && healthconnect.datetime.now_date() <= frm.doc.encounter_date) {
	...
	get_applicable_treatment_plans: function(frm) {
		healthconnect.call({
			...
			callback: function() {
				new healthconnect.ui.form.MultiSelectDialog({
...
var schedule_inpatient = function(frm) {
	var dialog = new healthconnect.ui.Dialog({
...
let create_procedure = function(frm) {
	...
	healthconnect.new_doc('Clinical Procedure');
};

let create_nursing_tasks = function(frm) {
	const d = new healthconnect.ui.Dialog({
		...
		fields: [
			...
			},
			{
				...
				default: healthconnect.datetime.now_datetime(),
				reqd: 1,
			},
		],
...
healthconnect.ui.form.on('Drug Prescription', {
	dosage: function(frm, cdt, cdn){
		healthconnect.model.set_value(cdt, cdn, 'update_schedule', 1);
		let child = locals[cdt][cdn];
		if (child.dosage) {
			healthconnect.model.set_value(cdt, cdn, 'interval_uom', 'Day');
			healthconnect.model.set_value(cdt, cdn, 'interval', 1);
		}
	},
	...
	interval_uom: function(frm, cdt, cdn) {
		...
		if (child.interval_uom == 'Hour') {
			healthconnect.model.set_value(cdt, cdn, 'dosage', null);
		}
	}
});
