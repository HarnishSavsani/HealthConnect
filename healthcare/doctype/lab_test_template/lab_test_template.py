...
from healthconnect.utils import flt, today
...
class LabTestTemplate(Document):
	def before_insert(self):
		if self.link_existing_item and self.item:
			price_list = healthconnect.db.get_all(
	...
	def validate(self):
		if (
			...
			healthconnect.throw(_("Standard Selling Rate should be greater than zero."))
	...
	def on_trash(self):
		...
		if self.item:
			try:
				item = self.item
				self.db_set("item", "")
				healthconnect.delete_doc("Item", item)
			except Exception:
				healthconnect.throw(_("Not permitted. Please disable the Lab Test Template"))
	...
	def update_item(self):
		item = healthconnect.get_doc("Item", self.item)
		if item:
			item.update(
				{
	...
	def item_price_exists(self):
		item_price = healthconnect.db.exists("Item Price", {"item_code": self.item, "valid_from": today()})
		if item_price:
			return item_price
		return False
	...
	def validate_conversion_factor(self):
		if self.lab_test_template_type == "Single" and self.secondary_uom and not self.conversion_factor:
			healthconnect.throw(_("Conversion Factor is mandatory"))
		if self.lab_test_template_type == "Compound":
			for item in self.normal_test_templates:
				if item.secondary_uom and not item.conversion_factor:
					healthconnect.throw(_("Row #{0}: Conversion Factor is mandatory").format(item.idx))
		if self.lab_test_template_type == "Grouped":
			for group in self.lab_test_groups:
				if (
					...
					healthconnect.throw(_("Row #{0}: Conversion Factor is mandatory").format(group.idx))
...
def create_item_from_template(doc):
	...
	if doc.is_billable and doc.lab_test_rate != 0.0:
		price_list_name = healthconnect.db.get_value(
			"Selling Settings", None, "selling_price_list"
		) or healthconnect.db.get_value("Price List", {"selling": 1})
		if doc.lab_test_rate:
			make_item_price(item.name, doc.lab_test_rate)
		else:
			make_item_price(item.name, 0.0)
	# Set item in the template
	healthconnect.db.set_value("Lab Test Template", doc.name, "item", item.name)
...
def change_test_code_from_template(lab_test_code, doc):
	...
	if healthconnect.db.exists({"doctype": "Item", "item_code": lab_test_code}):
		healthconnect.throw(_("Lab Test Item {0} already exist").format(lab_test_code))
	else:
