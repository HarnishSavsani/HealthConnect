...
from healthconnect.utils.nestedset import NestedSet
...
class HealthcareServiceUnit(NestedSet):
	...
	def autoname(self):
		if self.company:
			suffix = " - " + healthconnect.get_cached_value("Company", self.company, "abbr")
	...
	def set_service_unit_properties(self):
		if cint(self.is_group):
			...
			service_unit_type = healthconnect.get_doc("Healthcare Service Unit Type", self.service_unit_type)
...
def add_multiple_service_units(parent, data):
	...
	company = (
		data.get("company")
		or healthconnect.defaults.get_defaults().get("company")
		or healthconnect.db.get_single_value("Global Defaults", "default_company")
	)
	...
	last_suffix = healthconnect.db.sql(
		"""SELECT
	...
	for i in range(start_suffix, count + start_suffix):
		...
		service_unit_doc = healthconnect.get_doc(service_unit)
		try:
