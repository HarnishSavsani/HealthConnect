healthconnect.provide("healthconnect.treeview_settings");

healthconnect.treeview_settings['Healthcare Service Unit'] = {
			...
			onchange: () => {
				if (cur_dialog) {
					if (cur_dialog.fields_dict.service_unit_type.value) {
						healthconnect.db.get_value('Healthcare Service Unit Type',
	...
	post_render: function (treeview) {
		healthconnect.treeview_settings['Healthcare Service Unit'].treeview = {};
		$.extend(healthconnect.treeview_settings['Healthcare Service Unit'].treeview, treeview);
	},
	toolbar: [
		{
			...
			click: function (node) {
				const dialog = new healthconnect.ui.Dialog({
					...
					primary_action: () => {
						...
						return healthconnect.call({
							...
							callback: function (r) {
								if (!r.exc && r.message) {
									healthconnect.treeview_settings['Healthcare Service Unit'].treeview.tree.load_children(node, true);
									...
									healthconnect.msgprint(__('Could not create Service Units'));
								}
							},
							freeze: true,
