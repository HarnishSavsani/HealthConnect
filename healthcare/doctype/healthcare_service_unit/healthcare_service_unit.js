...
healthconnect.ui.form.on('Healthcare Service Unit', {
	onload: function(frm) {
	...
	refresh: function(frm) {
		...
		if (!frm.is_new()) {
			...
			healthconnect.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Healthcare Service Unit'}
		} else {
		...
		frm.add_custom_button(__('Healthcare Service Unit Tree'), function() {
			healthconnect.set_route('Tree', 'Healthcare Service Unit');
		});
...
healthconnect.ui.form.on('Healthcare Service Unit', 'parent_healthcare_service_unit', function(frm) {
	if (!frm.doc.parent_healthcare_service_unit) {
	    frm.set_intro(__('This is a root healthcare service unit and cannot be edited.'), true);
	    frm.set_read_only();
	    frm.disable_save();
	    ...
	    frm.set_intro('');
	    frm.perm = healthconnect.perm.get_perm(frm.doc.doctype);
	    frm.enable_save();
	}
});
