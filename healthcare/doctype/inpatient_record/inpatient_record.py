...
from healthconnect.desk.reportview import get_match_cond
from healthconnect.model.document import Document
from healthconnect.utils import get_datetime, get_link_to_form, getdate, now_datetime, today
...
class InpatientRecord(Document):
	def after_insert(self):
		healthconnect.db.set_value("Patient", self.patient, "inpatient_record", self.name)
		healthconnect.db.set_value("Patient", self.patient, "inpatient_status", self.status)
	...
	def validate_dates(self):
		if (getdate(self.expected_discharge) < getdate(self.scheduled_date)) or (
			...
			healthconnect.throw(_("Expected and Discharge dates cannot be less than Admission Schedule date"))
	...
	def validate_already_scheduled_or_admitted(self):
		...
		ip_record = healthconnect.db.sql(query, {"name": self.name, "patient": self.patient}, as_dict=1)
...
def schedule_inpatient(args):
	...
	if (
		...
		healthconnect.throw(_("Missing required details, did not create Inpatient Record"))
	...
	# Patient details
	patient = healthconnect.get_doc("Patient", admission_order["patient"])
	...
	encounter = healthconnect.get_doc("Patient Encounter", admission_order["admission_encounter"])
...
def schedule_discharge(args):
	...
	inpatient_record_id = healthconnect.db.get_value(
		"Patient", discharge_order["patient"], "inpatient_record"
	)

	if inpatient_record_id:

		inpatient_record = healthconnect.get_doc("Inpatient Record", inpatient_record_id)
...
def validate_inpatient_invoicing(inpatient_record):
	if healthconnect.db.get_single_value("Healthcare Settings", "allow_discharge_despite_unbilled_services"):
		return
	...
	if pending_invoices:
		...
		healthconnect.throw(message, title=_("Unbilled Services"), is_minimizable=True, wide=True)
...
def get_unbilled_inpatient_docs(doc, inpatient_record):
	return healthconnect.db.get_list(
		doc,
		filters={
...
def transfer_patient(inpatient_record, service_unit, check_in):
	...
	healthconnect.db.set_value("Healthcare Service Unit", service_unit, "occupancy_status", "Occupied")
...
def is_service_unit_billable(service_unit):
	service_unit_doc = healthconnect.qb.DocType("Healthcare Service Unit")
	service_unit_type = healthconnect.qb.DocType("Healthcare Service Unit Type")
	result = (
		healthconnect.qb.from_(service_unit_doc)
...
def set_ip_order_cancelled(inpatient_record, reason, encounter=None):
	inpatient_record = healthconnect.get_doc("Inpatient Record", inpatient_record)
