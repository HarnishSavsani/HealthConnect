...
healthconnect.ui.form.on('Inpatient Record', {
	setup: function(frm) {
	...
	let dialog = new healthconnect.ui.Dialog({
		...
		fields: [
			...
			{fieldtype: 'Datetime', label: 'Admission Datetime', fieldname: 'check_in',
				reqd: 1, default: healthconnect.datetime.now_datetime()
			},
			{fieldtype: 'Date', label: 'Expected Discharge', fieldname: 'expected_discharge',
				default: frm.doc.expected_length_of_stay ? healthconnect.datetime.add_days(healthconnect.datetime.now_datetime(), frm.doc.expected_length_of_stay) : ''
			}
		],
...
let transfer_patient_dialog = function(frm) {
	let dialog = new healthconnect.ui.Dialog({
		...
		fields: [
			...
			{fieldtype: 'Datetime', label: 'Check In', fieldname: 'check_in', reqd: 1, default: healthconnect.datetime.now_datetime()}
		],
		...
		primary_action : function() {
			...
			if(check_in > healthconnect.datetime.now_datetime()){
