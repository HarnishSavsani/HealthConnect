...
class TherapySession(Document):
	...
	def validate_duplicate(self):
		...
		if overlaps:
			...
			healthconnect.throw(overlapping_details, title=_("Therapy Sessions Overlapping"))
	...
	def create_nursing_tasks(self, post_event=True):
		template = healthconnect.db.get_value("Therapy Type", self.therapy_type, "nursing_checklist_template")
		if template:
	...
	def update_sessions_count_in_therapy_plan(self, on_cancel=False):
		therapy_plan = healthconnect.get_doc("Therapy Plan", self.therapy_plan)
	...
	def set_exercises_from_therapy_type(self):
		if self.therapy_type and not self.exercises:
			therapy_type_doc = healthconnect.get_cached_doc("Therapy Type", self.therapy_type)
			if therapy_type_doc.exercises:
				for exercise in therapy_type_doc.exercises:
					self.append(
						"exercises",
						(healthconnect.copy_doc(exercise)).as_dict(),
					)
...
def create_therapy_session(source_name, target_doc=None):
	def set_missing_values(source, target):
		therapy_type = healthconnect.get_doc("Therapy Type", source.therapy_type)
...
def invoice_therapy_session(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.customer = healthconnect.db.get_value("Patient", source.patient, "customer")
...
def get_therapy_item(therapy, item):
	item.item_code = healthconnect.db.get_value("Therapy Type", therapy.therapy_type, "item")
	...
	item.cost_center = healthconnect.get_cached_value("Company", therapy.company, "cost_center")
