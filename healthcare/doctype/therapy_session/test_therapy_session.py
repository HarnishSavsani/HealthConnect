...
class TestTherapySession(healthconnectTestCase):
	def test_exercise_set_from_therapy_type(self):
		...
		if plan.therapy_plan_details:
			therapy_type = healthconnect.get_doc("Therapy Type", plan.therapy_plan_details[0].therapy_type)
			self.assertEqual(
