...
from healthconnect.utils import flt, getdate, nowdate
...
class TestTherapyPlan(healthconnectTestCase):
	def test_creation_on_encounter_submission(self):
		patient, practitioner = create_healthcare_docs()
		medical_department = create_medical_department()
		encounter = create_encounter(patient, medical_department, practitioner)
		self.assertTrue(healthconnect.db.exists("Therapy Plan", encounter.therapy_plan))
	...
	def test_status(self):
		...
		healthconnect.get_doc(session).submit()
		self.assertEqual(healthconnect.db.get_value("Therapy Plan", plan.name, "status"), "In Progress")

		session = make_therapy_session(plan.name, plan.patient, "Basic Rehab", "_Test Company")
		healthconnect.get_doc(session).submit()
		self.assertEqual(healthconnect.db.get_value("Therapy Plan", plan.name, "status"), "Completed")
		...
		session = healthconnect.get_doc(session)
		session.submit()
		self.assertEqual(
	...
	def test_therapy_plan_from_template(self):
		...
		# check linked item
		self.assertTrue(healthconnect.db.exists("Therapy Plan Template", {"linked_item": "Complete Rehab"}))
		...
		therapy_plan_template_amt = healthconnect.db.get_value(
...
def create_therapy_plan(template=None, patient=None):
	...
	plan = healthconnect.new_doc("Therapy Plan")
...
def create_therapy_plan_template():
	template_name = healthconnect.db.exists("Therapy Plan Template", "Complete Rehab")
	if not template_name:
		therapy_type = create_therapy_type()
		template = healthconnect.new_doc("Therapy Plan Template")
		template.plan_name = template.item_code = template.item_name = "Complete Rehab"
		template.item_group = "Services"
		rate = healthconnect.db.get_value("Therapy Type", therapy_type.name, "rate")
		template.append(
			"therapy_types",
