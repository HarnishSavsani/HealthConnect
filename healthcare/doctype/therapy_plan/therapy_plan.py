...
class TherapyPlan(Document):
	...
	def set_therapy_details_from_template(self):
		...
		therapy_plan_template = healthconnect.get_doc("Therapy Plan Template", self.therapy_plan_template)
...
def make_therapy_session(therapy_plan, patient, therapy_type, company, appointment=None):
	...
	therapy_session = healthconnect.new_doc("Therapy Session")
	...
	if healthconnect.flags.in_test:
...
def make_sales_invoice(reference_name, patient, company, therapy_plan_template):
	...
	si = healthconnect.new_doc("Sales Invoice")
	si.company = company
	si.patient = patient
	si.customer = healthconnect.db.get_value("Patient", patient, "customer")

	item = healthconnect.db.get_value("Therapy Plan Template", therapy_plan_template, "linked_item")
