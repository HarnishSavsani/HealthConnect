...
from healthconnect.utils import today
...
def get_item_details(args=None):
	...
	item = healthconnect.db.get_all(
		"Item", filters={"disabled": 0, "name": args.get("item_code")}, fields=["stock_uom", "item_name"]
	)

	if not item:
		healthconnect.throw(_("Item {0} is not active").format(args.get("item_code")))
...
def change_item_code_from_template(item_code, doc):
	...
	if healthconnect.db.exists("Item", {"item_code": item_code}):
		...
		healthconnect.db.set_value("Clinical Procedure Template", doc.name, "item_code", item_code)
	return
...
def update_item_and_item_price(doc):
	if doc.is_billable and doc.item:
		...
		item_doc = healthconnect.get_doc("Item", {"item_code": doc.item})
		...
		if rate:
			if not healthconnect.db.exists("Item Price", {"item_code": doc.item, "valid_from": today()}):
				make_item_price(doc.item, rate)
			else:
				item_price = healthconnect.get_doc("Item Price", {"item_code": doc.item})
		...
		healthconnect.db.set_value("Item", doc.item, "disabled", 1)
