...
class HealthcareServiceUnitType(Document):
		...
		if self.allow_appointments and self.inpatient_occupancy:
			healthconnect.msgprint(
				_("Healthcare Service Unit Type cannot have both {0} and {1}").format(
					healthconnect.bold("Allow Appointments"), healthconnect.bold("Inpatient Occupancy")
				),
				raise_exception=1,
	...
	def on_trash(self):
		if self.item:
			try:
				...
				healthconnect.throw(_("Not permitted. Please disable the Service Unit Type"))
	...
	def on_update(self):
		if self.change_in_item and self.is_billable and self.item:
			...
			if not item_price:
				price_list_name = healthconnect.db.get_value("Price List", {"selling": 1})
				if self.rate:
					make_item_price(self.item_code, price_list_name, self.rate)
				else:
					make_item_price(self.item_code, price_list_name, 0.0)
			else:
				healthconnect.db.set_value("Item Price", item_price, "price_list_rate", self.rate)

			healthconnect.db.set_value(self.doctype, self.name, "change_in_item", 0)
...
def item_price_exists(doc):
	item_price = healthconnect.db.exists({"doctype": "Item Price", "item_code": doc.item_code})
	if len(item_price):
...
def update_item(doc):
	item = healthconnect.get_doc("Item", doc.item)
	if item:
		item.update(
			{
				"item_name": doc.service_unit_type,
...
def change_item_code(item, item_code, doc_name):
	if healthconnect.db.exists({"doctype": "Item", "item_code": item_code}):
		...
		healthconnect.db.set_value("Healthcare Service Unit Type", doc_name, "item_code", item_code)
