...
from healthconnect.utils import add_to_date, get_datetime, getdate, now_datetime, time_diff_in_seconds
...
class NursingTask(Document):
	def before_insert(self):
		...
		self.age = healthconnect.get_doc("Patient", self.patient).get_age()
	...
	def on_cancel(self):
		if self.status == "Completed":
			healthconnect.throw(_("Not Allowed to cancel Nursing Task with status 'Completed'"))

		self.db_set("status", "Cancelled")

	def on_update_after_submit(self):
		if self.status == "Completed" and self.task_doctype and not self.task_document_name:
			healthconnect.throw(_("Not Allowed to 'Complete' Nursing Task without linking Task Document"))

		if self.status == "Draft":
			healthconnect.throw(_("Nursing Task cannot be 'Draft' after submission"))
	...
	def set_task_schedule(self):
		...
		if not self.requested_end_time:
			if not self.duration:
				self.duration = healthconnect.db.get_value("Healthcare Activity", self.activity, "duration")
	...
	def create_nursing_tasks(cls, tasks, doc, start_time, post_event=True):
		for task in tasks:
			...
			if doc.get("doctype") == "Inpatient Record":
				service_unit = (
					healthconnect.db.get_value("Inpatient Occupancy", {"parent": doc.name, "left": 0}, "service_unit"),
				)
			else:
				service_unit = (
			...
			healthconnect.get_doc(options).insert()

	@classmethod
	def cancel_nursing_tasks(cls, dt, dn):
		tasks = healthconnect.db.get_all(
			"Nursing Task",
			filters={
		...
		for task in tasks:
			healthconnect.get_doc("Nursing Task", task).cancel()
