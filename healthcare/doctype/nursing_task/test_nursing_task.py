...
from healthconnect.utils import now_datetime
...
class TestNursingTask(healthconnectTestCase):
	def setUp(self) -> None:
		nursing_checklist_templates = healthconnect.get_test_records("Nursing Checklist Template")
		self.nc_template = healthconnect.get_doc(nursing_checklist_templates[0]).insert(
			ignore_if_duplicate=True
		)

		self.settings = healthconnect.get_single("Healthcare Settings")
	...
	def test_start_clinical_procedure_should_validate_pending_nursing_tasks(self):
		...
		self.assertRaises(healthconnect.ValidationError, procedure.start_procedure)
	...
	def test_admit_inpatient_should_validate_pending_nursing_tasks(self):
		...
		self.assertRaises(healthconnect.ValidationError, admit_patient, **kwargs)
		...
		self.assertRaises(healthconnect.ValidationError, discharge_patient, inpatient_record=ip_record)
	...
	def test_submit_therapy_session_should_validate_pending_nursing_tasks(self):
		...
		self.assertRaises(healthconnect.ValidationError, therapy_session.submit)
...
def complete_nusing_tasks(document):
	...
	for task_name in tasks:
		task = healthconnect.get_doc("Nursing Task", task_name)
