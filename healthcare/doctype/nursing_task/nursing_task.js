...
healthconnect.ui.form.on('Nursing Task', {
	...
	onload: function(frm) {
		...
		if (frm.doc.task_doctype) {
			...
			// set filters
			let filters = {};
			healthconnect.model.with_doctype(frm.doc.task_doctype, function() {
				if (healthconnect.meta.has_field(frm.doc.task_doctype, 'patient')) {
	...
	show_form_route_button: function(frm) {
		...
		if (!frm.is_new() && !frm.is_dirty() && frm.doc.task_doctype) {
			...
			frm.add_custom_button(__(`New ${frm.doc.task_doctype}`), () => {
				...
				healthconnect.new_doc(frm.doc.task_doctype);
			});
