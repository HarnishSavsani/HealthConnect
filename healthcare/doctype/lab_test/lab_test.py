...
from healthconnect.utils import get_link_to_form, getdate, now_datetime
...
class LabTest(Document):
	...
	def after_insert(self):
		if self.prescription:
			healthconnect.db.set_value("Lab Prescription", self.prescription, "lab_test_created", 1)
			if healthconnect.db.get_value("Lab Prescription", self.prescription, "invoiced"):
				self.invoiced = True
		if self.template:
			...
			template = healthconnect.db.get_value("Lab Test Template", self.template, "nursing_checklist_template")
			if template:
				...
				if not item.result_value and not item.allow_blank and item.require_result_value:
					healthconnect.throw(
						_("Row #{0}: Please enter the result value for {1}").format(
							item.idx, healthconnect.bold(item.lab_test_name)
						),
		...
		if self.descriptive_test_items:
			for item in self.descriptive_test_items:
				if not item.result_value and not item.allow_blank and item.require_result_value:
					healthconnect.throw(
						_("Row #{0}: Please enter the result value for {1}").format(
							item.idx, healthconnect.bold(item.lab_test_particulars)
						),
						title=_("Mandatory Results"),
					)
...
def create_test_from_template(lab_test):
	template = healthconnect.get_doc("Lab Test Template", lab_test.template)
	patient = healthconnect.get_doc("Patient", lab_test.patient)
...
def update_status(status, name):
	if name and status:
		healthconnect.db.set_value("Lab Test", name, {"status": status, "approved_date": getdate()})
...
def create_multiple(doctype, docname):
	...
	if lab_test_created:
		...
		healthconnect.msgprint(_("No Lab Tests created"))
		...
		for item in encounter.lab_test_prescription:
			if not item.lab_test_created:
				...
				if template:
					...
					healthconnect.db.set_value("Lab Prescription", item.name, "lab_test_created", 1)
...
def create_lab_test_from_invoice(sales_invoice):
	...
	invoice = healthconnect.get_doc("Sales Invoice", sales_invoice)
	if invoice and invoice.patient:
		patient = healthconnect.get_doc("Patient", invoice.patient)
		for item in invoice.items:
			lab_test_created = 0
			if item.reference_dt == "Lab Prescription":
				lab_test_created = healthconnect.db.get_value(
			...
			if lab_test_created != 1:
				...
				if template:
					...
					lab_test.save(ignore_permissions=True)
					if item.reference_dt != "Lab Prescription":
						healthconnect.db.set_value("Sales Invoice Item", item.name, "reference_dt", "Lab Test")
						healthconnect.db.set_value("Sales Invoice Item", item.name, "reference_dn", lab_test.name)
...
def get_lab_test_template(item):
	template_id = healthconnect.db.exists("Lab Test Template", {"item": item})
	if template_id:
		return healthconnect.get_doc("Lab Test Template", template_id)
	return False
...
def create_lab_test_doc(
	...
	lab_test = healthconnect.new_doc("Lab Test")
...
def create_sample_doc(template, patient, invoice, company=None):
	if template.sample:
		sample_exists = healthconnect.db.exists(
			{
		...
		if sample_exists:
			...
			sample_collection = healthconnect.get_doc("Sample Collection", sample_exists)
			...
			healthconnect.db.set_value("Sample Collection", sample_collection.name, "sample_qty", quantity)

		else:
			# Create Sample Collection for template, copy vals from Invoice
			sample_collection = healthconnect.new_doc("Sample Collection")
			if invoice:
...
def create_sample_collection(lab_test, template, patient, invoice):
	if healthconnect.get_cached_value("Healthcare Settings", None, "create_sample_collection_for_lab_test"):
...
def load_result_format(lab_test, template, prescription, invoice):
	if template.lab_test_template_type == "Single":
		...
		for lab_test_group in template.lab_test_groups:
			...
			if lab_test_group.lab_test_template:
				template_in_group = healthconnect.get_doc("Lab Test Template", lab_test_group.lab_test_template)
	...
	if template.lab_test_template_type != "No Result":
		if prescription:
			...
			if invoice:
				healthconnect.db.set_value("Lab Prescription", prescription, "invoiced", True)
...
def get_employee_by_user_id(user_id):
	emp_id = healthconnect.db.exists("Employee", {"user_id": user_id})
	if emp_id:
		return healthconnect.get_doc("Employee", emp_id)
	return None
