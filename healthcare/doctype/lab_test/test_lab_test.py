...
from healthconnect.utils import getdate, nowtime
...
class TestLabTest(healthconnectTestCase):
	def test_lab_test_item(self):
		lab_template = create_lab_test_template()
		self.assertTrue(healthconnect.db.exists("Item", lab_template.item))
		self.assertEqual(
			healthconnect.db.get_value("Item Price", {"item_code": lab_template.item}, "price_list_rate"),
			lab_template.lab_test_rate,
		)

		lab_template.disabled = 1
		lab_template.save()
		self.assertEqual(healthconnect.db.get_value("Item", lab_template.item, "disabled"), 1)
	...
	def test_descriptive_lab_test(self):
		...
		lab_test.save()
		self.assertRaises(healthconnect.ValidationError, lab_test.submit)
	...
	def test_sample_collection(self):
		...
		self.assertTrue(healthconnect.db.exists("Sample Collection", {"sample": lab_template.sample}))
...
def create_lab_test_template(test_sensitivity=0, sample_collection=1):
	...
	if healthconnect.db.exists("Lab Test Template", "Insulin Resistance"):
		return healthconnect.get_doc("Lab Test Template", "Insulin Resistance")
	template = healthconnect.new_doc("Lab Test Template")
...
def create_medical_department():
	medical_department = healthconnect.db.exists("Medical Department", "_Test Medical Department")
...
def create_lab_test_sample():
	blood_sample = healthconnect.db.exists("Lab Test Sample", "Blood Sample")
	if blood_sample:
		return blood_sample

	sample = healthconnect.new_doc("Lab Test Sample")
...
def create_patient_encounter():
	...
	patient_encounter = healthconnect.new_doc("Patient Encounter")
...
def create_practitioner():
	practitioner = healthconnect.db.exists("Healthcare Practitioner", "_Test Healthcare Practitioner")
