...
healthconnect.ui.form.on('Lab Test', {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (healthconnect.defaults.get_default('lab_test_approval_required') && healthconnect.user.has_role('LabTest Approver')) {
...
healthconnect.ui.form.on('Lab Test', 'patient', function (frm) {
	if (frm.doc.patient) {
		healthconnect.call({
...
healthconnect.ui.form.on('Normal Test Result', {
	normal_test_items_remove: function () {
		healthconnect.msgprint(__('Not permitted, configure Lab Test Template as required'));
		cur_frm.reload_doc();
	}
});

healthconnect.ui.form.on('Descriptive Test Result', {
...
var get_lab_test_prescribed = function (frm) {
	if (frm.doc.patient) {
		...
		healthconnect.msgprint(__('Please select Patient to get Lab Tests'));
	}
};
...
var send_sms = function (vals, frm) {
	...
	if (!number || !message) {
		healthconnect.throw(__('Did not send SMS, missing patient mobile number or message content.'));
	}
	healthconnect.call({
		method: 'healthconnect.core.doctype.sms_settings.sms_settings.send_sms',
		args: {
