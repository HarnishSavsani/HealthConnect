...
from healthconnect.utils import flt, get_link_to_form, get_time, getdate
...
class InpatientMedicationEntry(Document):
	...
	def get_medication_orders(self):
		...
		if orders:
			...
			healthconnect.msgprint(_("No pending medication orders found for selected criteria"))
	...
	def on_submit(self):
		...
		if self.update_stock:
			...
			success_msg += _("Stock Entry {0} created and ").format(
				healthconnect.bold(get_link_to_form("Stock Entry", stock_entry))
			)

		self.update_medication_orders()
		success_msg += _("Inpatient Medication Orders updated successfully")
		healthconnect.msgprint(success_msg, title=_("Success"), indicator="green")

	def validate_medication_orders(self):
		for entry in self.medication_orders:
			docstatus, is_completed = healthconnect.db.get_value(
	...
	def process_stock(self):
		allow_negative_stock = healthconnect.db.get_single_value("Stock Settings", "allow_negative_stock")
	...
	def update_medication_orders(self, on_cancel=False):
		...
		for order, count in order_entry_map.items():
			medication_order = healthconnect.get_doc("Inpatient Medication Order", order)
			completed_orders = flt(count)
			current_value = healthconnect.db.get_value("Inpatient Medication Order", order, "completed_orders")
	...
	def check_stock_qty(self):
		...
		if drug_shortage:
			message = _("Quantity not available for the following items in warehouse {0}. ").format(
				healthconnect.bold(self.warehouse)
			)
			message += _(
			...
			for drug, shortage_qty in drug_shortage.items():
				...
				formatted_item_rows += """
					...
					item_link, healthconnect.bold(shortage_qty)
				)
			...
			healthconnect.throw(message, title=_("Insufficient Stock"), is_minimizable=True, wide=True)

	def make_stock_entry(self):
		stock_entry = healthconnect.new_doc("Stock Entry")
		stock_entry.purpose = "Material Issue"
		stock_entry.set_stock_entry_type()
		stock_entry.from_warehouse = self.warehouse
		stock_entry.company = self.company
		stock_entry.inpatient_medication_entry = self.name
		cost_center = healthconnect.get_cached_value("Company", self.company, "cost_center")
		...
		for entry in self.medication_orders:
			...
			se_child.uom = healthconnect.db.get_value("Item", entry.drug_code, "stock_uom")
	...
	def cancel_stock_entries(self):
		stock_entries = healthconnect.get_all("Stock Entry", {"inpatient_medication_entry": self.name})
		for entry in stock_entries:
			doc = healthconnect.get_doc("Stock Entry", entry.name)
			doc.cancel()
...
def get_current_healthcare_service_unit(inpatient_record):
	ip_record = healthconnect.get_doc("Inpatient Record", inpatient_record)
...
def make_difference_stock_entry(docname):
	doc = healthconnect.get_doc("Inpatient Medication Entry", docname)
	...
	cost_center = healthconnect.get_cached_value("Company", doc.company, "cost_center")
	expense_account = get_account(None, "expense_account", "Healthcare Settings", doc.company)

	for drug, shortage_qty in drug_shortage.items():
		se_child = stock_entry.append("items")
		se_child.item_code = drug
		se_child.item_name = healthconnect.db.get_value("Item", drug, "stock_uom")
		se_child.uom = healthconnect.db.get_value("Item", drug, "stock_uom")
		se_child.stock_uom = se_child.uom
