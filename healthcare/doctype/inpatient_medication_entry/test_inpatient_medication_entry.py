...
class TestInpatientMedicationEntry(healthconnectTestCase):
	def setUp(self):
		healthconnect.db.sql("""delete from `tabInpatient Record`""")
		healthconnect.db.sql("""delete from `tabInpatient Medication Order`""")
		healthconnect.db.sql("""delete from `tabInpatient Medication Entry`""")
	...
	def test_ipme_with_stock_update(self):
		...
		is_order_completed = healthconnect.db.get_value(
			"Inpatient Medication Order Entry", ipme.medication_orders[0].against_imoe, "is_completed"
		)
		self.assertEqual(is_order_completed, 1)

		# test stock entry
		stock_entry = healthconnect.db.exists("Stock Entry", {"inpatient_medication_entry": ipme.name})
		self.assertTrue(stock_entry)

		# check references
		stock_entry = healthconnect.get_doc("Stock Entry", stock_entry)
	...
	def tearDown(self):
		...
		for entry in healthconnect.get_all("Inpatient Medication Entry"):
			doc = healthconnect.get_doc("Inpatient Medication Entry", entry.name)
			doc.cancel()

		for entry in healthconnect.get_all("Inpatient Medication Order"):
			doc = healthconnect.get_doc("Inpatient Medication Order", entry.name)
			doc.cancel()
