...
class TestPatientMedicalRecord(healthconnectTestCase):
	def setUp(self):
...
def create_procedure(appointment):
	if appointment:
		procedure = healthconnect.new_doc("Clinical Procedure")
...
def create_vital_signs(appointment):
	vital_signs = healthconnect.new_doc("Vital Signs")
...
def create_lab_test_template(medical_department):
	if healthconnect.db.exists("Lab Test Template", "Blood Test"):
		return healthconnect.get_doc("Lab Test Template", "Blood Test")
...
def create_lab_test(template, patient):
	...
	lab_test.patient_sex = healthconnect.db.get_value("Patient", patient, "sex")
