...
from healthconnect.contacts.address_and_contact import load_address_and_contact
from healthconnect.contacts.doctype.contact.contact import get_default_contact
from healthconnect.model.document import Document
from healthconnect.model.naming import set_name_by_naming_series
from healthconnect.utils import cint, cstr, getdate
from healthconnect.utils.nestedset import get_root_of
...
class Patient(Document):
	...
	def after_insert(self):
		if healthconnect.db.get_single_value("Healthcare Settings", "collect_registration_fee"):
			healthconnect.db.set_value("Patient", self.name, "status", "Disabled")
		else:
			send_registration_sms(self)
		self.reload()

	def on_update(self):
		if healthconnect.db.get_single_value("Healthcare Settings", "link_customer_to_patient"):
			if self.customer:
				customer = healthconnect.get_doc("Customer", self.customer)
	...
	def set_missing_customer_details(self):
		if not self.customer_group:
			self.customer_group = healthconnect.db.get_single_value(
				"Selling Settings", "customer_group"
			) or get_root_of("Customer Group")
		if not self.territory:
			self.territory = healthconnect.db.get_single_value("Selling Settings", "territory") or get_root_of(
				"Territory"
			)
		if not self.default_price_list:
			self.default_price_list = healthconnect.db.get_single_value("Selling Settings", "selling_price_list")
		...
		if not self.language:
			self.language = healthconnect.db.get_single_value("System Settings", "language")

	def create_website_user(self):
		users = healthconnect.db.get_all(
			"User",
		...
		if users and users[0]:
			healthconnect.throw(
				...
				).format(healthconnect.bold(users[0].email), healthconnect.bold(users[0].mobile_no)),
	...
	def autoname(self):
		patient_name_by = healthconnect.db.get_single_value("Healthcare Settings", "patient_name_by")
	...
	def get_patient_name(self):
		...
		if healthconnect.db.get_value("Patient", name):
			count = healthconnect.db.sql(
	...
	def invoice_patient_registration(self):
		if healthconnect.db.get_single_value("Healthcare Settings", "registration_fee"):
			company = healthconnect.defaults.get_user_default("company")
			if not company:
				company = healthconnect.db.get_single_value("Global Defaults", "default_company")

			sales_invoice = make_invoice(self.name, company)
			sales_invoice.save(ignore_permissions=True)
			healthconnect.db.set_value("Patient", self.name, "status", "Active")
			send_registration_sms(self)
	...
	def update_contact(self, contact):
		contact = healthconnect.get_doc("Contact", contact)
...
def create_customer(doc):
	customer = healthconnect.get_doc(
		{
			"doctype": "Customer",
			"customer_name": doc.patient_name,
			"customer_group": doc.customer_group
			or healthconnect.db.get_single_value("Selling Settings", "customer_group"),
			"territory": doc.territory or healthconnect.db.get_single_value("Selling Settings", "territory"),
	...
	healthconnect.db.set_value("Patient", doc.name, "customer", customer.name)
	healthconnect.msgprint(_("Customer {0} is created.").format(customer.name), alert=True)
...
def make_invoice(patient, company):
	...
	sales_invoice.customer = healthconnect.db.get_value("Patient", patient, "customer")
	...
	item_line.rate = healthconnect.db.get_single_value("Healthcare Settings", "registration_fee")
...
def get_patient_detail(patient):
	patient_dict = healthconnect.db.sql("""select * from tabPatient where name=%s""", (patient), as_dict=1)
	if not patient_dict:
		healthconnect.throw(_("Patient not found"))
	vital_sign = healthconnect.db.sql(
...
def get_timeline_data(doctype, name):
	...
	customer = healthconnect.db.get_value(doctype, name, "customer")
	if customer:
