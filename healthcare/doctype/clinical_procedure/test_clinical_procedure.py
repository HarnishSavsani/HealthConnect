...
class TestClinicalProcedure(healthconnectTestCase):
	def test_procedure_template_item(self):
		patient, practitioner = create_healthcare_docs()
		procedure_template = create_clinical_procedure_template()
		self.assertTrue(healthconnect.db.exists("Item", procedure_template.item))

		procedure_template.disabled = 1
		procedure_template.save()
		self.assertEqual(healthconnect.db.get_value("Item", procedure_template.item, "disabled"), 1)
	...
	def test_consumables(self):
		...
		# check consumption
		self.assertTrue(healthconnect.db.exists("Stock Entry", result))


def create_consumable():
	if healthconnect.db.exists("Item", "Syringe"):
		return healthconnect.get_doc("Item", "Syringe")
	consumable = healthconnect.new_doc("Item")
