...
healthconnect.ui.form.on('Clinical Procedure', {
	setup: function(frm) {
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			let item = locals[cdt][cdn];
			if (!item.item_code) {
				healthconnect.throw(__('Please enter Item Code to get Batch Number'));
			} else {
				let filters = {'item_code': item.item_code};

				if (frm.doc.status == 'In Progress') {
					filters['posting_date'] = frm.doc.start_date || healthconnect.datetime.nowdate();
										...
										function() {
											healthconnect.call({
												...
												callback: function(r) {
													if (!r.exc) {
														frm.reload_doc();
														let doclist = healthconnect.model.sync(r.message);
														healthconnect.set_route('Form', doclist[0].doctype, doclist[0].name);
													}
												}
											});
										}
	...
	practitioner: function(frm) {
		if (frm.doc.practitioner) {
			healthconnect.call({
				...
				callback: function (data) {
					healthconnect.model.set_value(frm.doctype,frm.docname, 'practitioner_name', data.message.practitioner_name);
				}
			});
		} else {
			healthconnect.model.set_value(frm.doctype,frm.docname, 'practitioner_name', '');
		}
	},
...
healthconnect.ui.form.on('Clinical Procedure Item', {
	qty: function(frm, cdt, cdn) {
		let d = locals[cdt][cdn];
		healthconnect.model.set_value(cdt, cdn, 'transfer_qty', d.qty*d.conversion_factor);
	},

	uom: function(doc, cdt, cdn) {
		let d = locals[cdt][cdn];
		if (d.uom && d.item_code) {
			return healthconnect.call({
				...
				callback: function(r) {
					if (r.message) {
						healthconnect.model.set_value(cdt, cdn, r.message);
					}
				}
			});
		}
