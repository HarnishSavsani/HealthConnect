...
from healthconnect.utils import flt, now_datetime, nowdate, nowtime
...
class ClinicalProcedure(Document):
	...
	def after_insert(self):
		if self.prescription:
			healthconnect.db.set_value("Procedure Prescription", self.prescription, "procedure_created", 1)
		if self.appointment:
			healthconnect.db.set_value("Patient Appointment", self.appointment, "status", "Closed")

		if self.procedure_template:
			template = healthconnect.get_doc("Clinical Procedure Template", self.procedure_template)
			if template.sample:
				patient = healthconnect.get_doc("Patient", self.patient)
				sample_collection = create_sample_doc(template, patient, None, self.company)
				healthconnect.db.set_value("Clinical Procedure", self.name, "sample", sample_collection.name)
	...
	def create_nursing_tasks(self, post_event=True):
		if post_event:
			template = healthconnect.db.get_value(
			...
			start_time = healthconnect.utils.get_datetime(f"{self.start_date} {self.start_time}")
	...
	def complete_procedure(self):
		...
		if self.items:
			...
			customer = healthconnect.db.get_value("Patient", self.patient, "customer")
			if customer:
				for item in self.items:
					if item.invoice_separately_as_consumables:
						price_list, price_list_currency = healthconnect.db.get_values(
				...
				healthconnect.throw(
					_("Please set Customer in Patient {0}").format(healthconnect.bold(self.patient)),
...
def get_items(table, parent, parenttype):
	items = healthconnect.db.get_all(
...
def make_procedure(source_name, target_doc=None):
	def set_missing_values(source, target):
		consume_stock = healthconnect.db.get_value(
		...
		if consume_stock:
			...
			if source.service_unit:
				warehouse = healthconnect.db.get_value("Healthcare Service Unit", source.service_unit, "warehouse")
			if not warehouse:
				warehouse = healthconnect.db.get_value("Stock Settings", None, "default_warehouse")
			if warehouse:
