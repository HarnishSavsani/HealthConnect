...
healthconnect.ui.form.on('Vital Signs', {
	height: function(frm) {
...
let calculate_bmi = function(frm){
	...
	healthconnect.model.set_value(frm.doctype,frm.docname, 'bmi', bmi);
	healthconnect.model.set_value(frm.doctype,frm.docname, 'nutrition_note', bmi_note);
};

let set_bp = function(frm){
	let bp = frm.doc.bp_systolic+ '/' + frm.doc.bp_diastolic + ' mmHg';
	healthconnect.model.set_value(frm.doctype,frm.docname, 'bp', bp);
};
