healthconnect.provide('healthconnect.patient_history');
healthconnect.pages['patient_history'].on_page_load = function(wrapper) {
...
class PatientHistory {
	...
	show() {
		...
		if (healthconnect.route_options && !this.patient_id) {
	...
	make_patient_profile() {
		...
		this.main_section.empty().append(healthconnect.render_template('patient_history'));
		this.setup_filters();
	...
	setup_filters() {
		...
		healthconnect.xcall(
			...
			let doctype_filter = healthconnect.ui.form.make_control({
	...
	add_to_records(data) {
		...
		for (i=0; i<data.length; i++) {
			if (data[i].reference_doctype) {
				...
				if (healthconnect.user_info(data[i].owner).image) {
					data[i].imgsrc = healthconnect.utils.get_file_link(healthconnect.user_info(data[i].owner).image);
				} else {
					data[i].imgsrc = false;
				}
				...
				time_line_heading += data[i].reference_doctype + " - " +
					`<a onclick="healthconnect.set_route('Form', '${data[i].reference_doctype}', '${data[i].reference_name}');">
				...
				details += `<div class='d-flex flex-column width-full'>
						...
						<div class='healthconnect-card p-5 mt-3'>
	...
	add_date_separator(data) {
		let date = healthconnect.datetime.str_to_obj(data.communication_date);
		let pdate = '';
		let diff = healthconnect.datetime.get_day_diff(healthconnect.datetime.get_today(),
			healthconnect.datetime.obj_to_str(date));
		...
		if (diff < 1) {
			...
			pdate = __('on {0}', [healthconnect.datetime.global_date_format(date)]);
		}
		data.date_sep = pdate;
		return data;
	}
	...
	show_patient_info() {
		this.get_patient_info().then(() => {
			$('.patient-info').empty().append(healthconnect.render_template('patient_history_sidebar', {
		...
		healthconnect.call({
			...
			callback: function(r) {
				if (r.message) {
					...
					new healthconnect.Chart('.patient_vital_charts', {
						data: {
							labels: labels,
							datasets: datasets
						},
