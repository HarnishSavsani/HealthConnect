...
from healthconnect.utils import get_timespan_date_range, getdate


@healthconnect.whitelist()
def get_therapy_sessions_count(patient):
	total = healthconnect.db.count("Therapy Session", filters={"docstatus": 1, "patient": patient})

	month_start = datetime.today().replace(day=1)
	this_month = healthconnect.db.count(
...
def get_therapy_sessions_distribution_data(patient, field):
	if field == "therapy_type":
		result = healthconnect.db.get_all(
			"Therapy Session",
