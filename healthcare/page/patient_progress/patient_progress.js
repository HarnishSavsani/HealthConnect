healthconnect.pages['patient-progress'].on_page_load = function(wrapper) {

	healthconnect.ui.make_app_page({
		parent: wrapper,
...
class PatientProgress {
	...
	show() {
		healthconnect.breadcrumbs.add('Healthcare');
		this.sidebar.empty();

		let me = this;
		let patient = healthconnect.ui.form.make_control({
			parent: me.sidebar,
			df: {
				fieldtype: 'Link',
				options: 'Patient',
		...
		if (healthconnect.route_options && !this.patient) {
			patient.set_value(healthconnect.route_options.patient);
			this.patient_id = healthconnect.route_options.patient;
		}
	...
	make_patient_profile() {
		...
		this.main_section.empty().append(healthconnect.render_template('patient_progress'));
	...
	get_patient_info() {
		return healthconnect.xcall('healthconnect.client.get', {
			doctype: 'Patient',
	...
	render_patient_details() {
		this.get_patient_info().then(() => {
			this.get_therapy_sessions_count().then(() => {
				$('.patient-info').empty().append(healthconnect.render_template('patient_progress_sidebar', {
	...
	setup_patient_profile_links() {
		this.wrapper.find('.patient-profile-link').on('click', () => {
			healthconnect.set_route('Form', 'Patient', this.patient_id);
		});

		this.wrapper.find('.therapy-plan-link').on('click', () => {
			healthconnect.route_options = {
				'patient': this.patient_id,
				'docstatus': 1
			};
			healthconnect.set_route('List', 'Therapy Plan');
		});
		...
		this.wrapper.find('.patient-history').on('click', () => {
			...
			healthconnect.set_route('patient_history');
		});
	}

	render_heatmap() {
		this.heatmap = new healthconnect.Chart('.patient-heatmap', {
			type: 'heatmap',
	...
	update_heatmap_data(date_from) {
		healthconnect.xcall('healthcare.healthcare.page.patient_progress.patient_progress.get_patient_heatmap_data', {
			patient: this.patient_id,
			date: date_from || healthconnect.datetime.year_start(),
		}).then((data) => {
	...
	create_heatmap_chart_filters() {
		this.get_patient_info().then(() => {
			let filters = [
				{
					label: healthconnect.dashboard_utils.get_year(healthconnect.datetime.now_date()),
					options: healthconnect.dashboard_utils.get_years_since_creation(this.patient.creation),
					action: (selected_item) => {
						this.update_heatmap_data(healthconnect.datetime.obj_to_str(selected_item));
					}
				},
			];
			healthconnect.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.heatmap-container');
		});
	}
	...
	render_percentage_chart(field, title) {
		...
		// 		this.percentage_chart = new healthconnect.Chart('.therapy-session-percentage-chart', {
		// 			title: title,
		// 			type: 'percentage',
		// 			data: {
		// 				labels: chart.labels,
	...
	create_percentage_chart_filters() {
		...
		healthconnect.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.percentage-chart-container');
	}
	...
	create_time_span_filters(action_method, parent) {
		...
		healthconnect.dashboard_utils.render_chart_filters(filters, 'chart-filter', chart_control, 1);
	}
	...
	render_date_range_fields(action_method, parent) {
		...
		if (!date_field.length) {
			...
			let date_range_field = healthconnect.ui.form.make_control({
				df: {
					fieldtype: 'DateRange',
					fieldname: 'from_date',
	...
	show_therapy_progress() {
		let me = this;
		let therapy_type = healthconnect.ui.form.make_control({
		...
		healthconnect.xcall(
			...
			if (!chart.labels.length) {
				...
				if (!this.therapy_line_chart) {
					this.therapy_line_chart = new healthconnect.Chart(parent, {
						type: 'axis-mixed',
						height: 250,
						data: data,
						lineOptions: {
	...
	show_assessment_results() {
		let me = this;
		let assessment_template = healthconnect.ui.form.make_control({
		...
		healthconnect.xcall(
			...
			if (!chart.labels.length) {
				...
				if (!this.assessment_line_chart) {
					this.assessment_line_chart = new healthconnect.Chart(parent, {
						type: 'axis-mixed',
						height: 250,
						data: data,
						lineOptions: {
	...
	show_therapy_assessment_correlation() {
		let me = this;
		let assessment = healthconnect.ui.form.make_control({
		...
		healthconnect.xcall(
			...
			if (!chart.labels.length) {
				...
				if (!this.correlation_chart) {
					this.correlation_chart = new healthconnect.Chart(parent, {
						type: 'axis-mixed',
						height: 300,
						data: data,
						axisOptions: {
	...
	show_assessment_parameter_progress() {
		let me = this;
		let parameter = healthconnect.ui.form.make_control({
		...
		healthconnect.xcall(
			...
			if (!chart.labels.length) {
				...
				if (!this.parameter_chart) {
					this.parameter_chart = new healthconnect.Chart(parent, {
						type: 'line',
						height: 250,
						data: data,
						lineOptions: {
