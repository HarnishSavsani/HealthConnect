...
from healthconnect.utils import cstr, get_link_to_form, rounded, time_diff_in_hours
from healthconnect.utils.formatters import format_value
...
def validate_customer_created(patient):
	if not healthconnect.db.get_value("Patient", patient.name, "customer"):
...
def get_appointments_to_invoice(patient, company):
	...
	patient_appointments = healthconnect.get_list(
		"Patient Appointment",
		fields="*",
		filters={
	...
	for appointment in patient_appointments:
		...
		# Consultation Appointments, should check fee validity
			...
			if healthconnect.db.get_single_value(
				"Healthcare Settings", "enable_free_follow_ups"
			) and healthconnect.db.exists("Fee Validity Reference", {"appointment": appointment.name}):
...
def get_encounters_to_invoice(patient, company):
	...
	encounters = healthconnect.get_list(
		"Patient Encounter",
		fields=["*"],
		...
		for encounter in encounters:
			if not encounter.appointment:
				...
				if encounter.practitioner:
					if encounter.inpatient_record and healthconnect.db.get_single_value(
...
def get_lab_tests_to_invoice(patient, company):
	...
	lab_prescriptions = healthconnect.db.sql(
		"""
			SELECT
...
def get_clinical_procedures_to_invoice(patient, company):
	...
	procedures = healthconnect.get_list(
		"Clinical Procedure",
		fields="*",
	...
	for procedure in procedures:
		...
		if (
			...
			service_item = healthconnect.db.get_single_value(
				"Healthcare Settings", "clinical_procedure_consumable_item"
			)
			if not service_item:
				healthconnect.throw(
					_("Please configure Clinical Procedure Consumable Item in {0}").format(
						healthconnect.utils.get_link_to_form("Healthcare Settings", "Healthcare Settings")
					),
	...
	procedure_prescriptions = healthconnect.db.sql(
		"""
			SELECT
...
def get_inpatient_services_to_invoice(patient, company):
	services_to_invoice = []
	inpatient_services = healthconnect.db.sql(
		"""
			SELECT
				io.*
			FROM
	...
	for inpatient_occupancy in inpatient_services:
		service_unit_type = healthconnect.db.get_value(
			"Healthcare Service Unit", inpatient_occupancy.service_unit, "service_unit_type"
		)
		service_unit_type = healthconnect.get_cached_doc("Healthcare Service Unit Type", service_unit_type)
...
def get_therapy_plans_to_invoice(patient, company):
	...
	therapy_plans = healthconnect.get_list(
		"Therapy Plan",
		...
		therapy_plans_to_invoice.append(
			{
				...
				"service": healthconnect.db.get_value(
...
def get_therapy_sessions_to_invoice(patient, company):
	...
	for therapy in therapy_sessions:
		if not therapy.appointment:
			if therapy.therapy_type and healthconnect.db.get_value(
				...
				therapy_sessions_to_invoice.append(
					{
						...
						"service": healthconnect.db.get_value("Therapy Type", therapy.therapy_type, "item"),
					}
				)
...
def throw_config_service_item(is_inpatient):
	...
	healthconnect.throw(msg, title=_("Missing Configuration"))
...
def get_practitioner_billing_details(practitioner, is_inpatient):
	...
	if practitioner:
		service_item, practitioner_charge = healthconnect.db.get_value(
...
def get_healthcare_service_item(is_inpatient):
	...
	if is_inpatient:
		service_item = healthconnect.db.get_single_value("Healthcare Settings", "inpatient_visit_charge_item")
	else:
		service_item = healthconnect.db.get_single_value("Healthcare Settings", "op_consulting_charge_item")
	...
	if doc.items:
		for item in doc.items:
			if item.get("reference_dt") and item.get("reference_dn"):
				if healthconnect.get_meta(item.reference_dt).has_field("invoiced"):
					set_invoiced(item, method, doc.name)

	if method == "on_submit":
		if healthconnect.db.get_single_value("Healthcare Settings", "create_lab_test_on_si_submit"):
			create_multiple("Sales Invoice", doc.name)

		if (
			not healthconnect.db.get_single_value("Healthcare Settings", "automate_appointment_invoicing")
			and healthconnect.db.get_single_value("Healthcare Settings", "enable_free_follow_ups")
			and doc.items
		):
			for item in doc.items:
				if item.reference_dt == "Patient Appointment":
					fee_validity = healthconnect.db.exists("Fee Validity", {"patient_appointment": item.reference_dn})
					if fee_validity:
						healthconnect.db.set_value("Fee Validity", fee_validity, "sales_invoice_ref", doc.name)
...
def set_invoiced(item, method, ref_invoice=None):
	...
	if item.reference_dt == "Clinical Procedure":
		...
		if service_item == item.item_code:
			healthconnect.db.set_value(item.reference_dt, item.reference_dn, "consumption_invoiced", invoiced)
		else:
			healthconnect.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)
	else:
		healthconnect.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)

	if item.reference_dt == "Patient Appointment":
		if healthconnect.db.get_value("Patient Appointment", item.reference_dn, "procedure_template"):
...
def validate_invoiced_on_submit(item):
	if (
		...
		and healthconnect.db.get_single_value("Healthcare Settings", "clinical_procedure_consumable_item")
		== item.item_code
	):
		is_invoiced = healthconnect.db.get_value(item.reference_dt, item.reference_dn, "consumption_invoiced")
	else:
		is_invoiced = healthconnect.db.get_value(item.reference_dt, item.reference_dn, "invoiced")
	if is_invoiced:
		healthconnect.throw(
...
def manage_prescriptions(invoiced, ref_dt, ref_dn, dt, created_check_field):
	created = healthconnect.db.get_value(ref_dt, ref_dn, created_check_field)
	if created:
		# Fetch the doc created for the prescription
		doc_created = healthconnect.db.get_value(dt, {"prescription": ref_dn})
		healthconnect.db.set_value(dt, doc_created, "invoiced", invoiced)


def manage_doc_for_appointment(dt_from_appointment, appointment, invoiced):
	dn_from_appointment = healthconnect.db.get_value(
		dt_from_appointment, filters={"appointment": appointment}
	)
	if dn_from_appointment:
		healthconnect.db.set_value(dt_from_appointment, dn_from_appointment, "invoiced", invoiced)


@healthconnect.whitelist()
def get_drugs_to_invoice(encounter):
	encounter = healthconnect.get_doc("Patient Encounter", encounter)
	if encounter:
		patient = healthconnect.get_doc("Patient", encounter.patient)
		if patient:
			if patient.customer:
				items_to_invoice = []
				for drug_line in encounter.drug_prescription:
					if drug_line.drug_code:
						qty = 1
						if healthconnect.db.get_value("Item", drug_line.drug_code, "stock_uom") == "Nos":
...
def get_patient_vitals(patient, from_date=None, to_date=None):
	...
	vitals = healthconnect.db.get_all(
		"Vital Signs",
...
def render_doc_as_html(doctype, docname, exclude_fields=None):
	...
	doc = healthconnect.get_doc(doctype, docname)
	meta = healthconnect.get_meta(doctype)
	...
	for df in meta.fields:
		...
		if df.fieldtype == "Table":
			...
			if not items:
				continue
			child_meta = healthconnect.get_meta(df.options)
...
def update_address_links(address, method):
	...
	"""
	if "Healthcare" not in healthconnect.get_active_domains():
		return

	patient_links = list(filter(lambda link: link.get("link_doctype") == "Patient", address.links))

	for link in patient_links:
		customer = healthconnect.db.get_value("Patient", link.get("link_name"), "customer")
...
def update_patient_email_and_phone_numbers(contact, method):
	...
	"""
	if "Healthcare" not in healthconnect.get_active_domains() or contact.flags.skip_patient_update:
		return
	...
	if contact.is_primary_contact and (contact.email_id or contact.mobile_no or contact.phone):
		...
		for link in patient_links:
			contact_details = healthconnect.db.get_value(
			...
			if contact.email_id and contact.email_id != contact_details.get("email"):
				healthconnect.db.set_value("Patient", link.get("link_name"), "email", contact.email_id)
			if contact.mobile_no and contact.mobile_no != contact_details.get("mobile"):
				healthconnect.db.set_value("Patient", link.get("link_name"), "mobile", contact.mobile_no)
			if contact.phone and contact.phone != contact_details.get("phone"):
				healthconnect.db.set_value("Patient", link.get("link_name"), "phone", contact.phone)


def before_tests():
	# complete setup if missing
	from healthconnect.desk.page.setup_wizard.setup_wizard import setup_complete

	current_year = healthconnect.utils.now_datetime().year

	if not healthconnect.get_list("Company"):
		setup_complete(
			{
				"currency": "INR",
				"full_name": "Test User",
				"company_name": "healthconnect Care LLC",
...
def validate_nursing_tasks(document):
	if not healthconnect.db.get_single_value("Healthcare Settings", "validate_nursing_checklists"):
		return True
	...
	tasks = healthconnect.get_all("Nursing Task", filters=filters)
	if not tasks:
		return True
...
def get_medical_codes(template_dt, template_dn, code_standard=None):
	...
	return healthconnect.db.get_all(
		"Codification Table",
		filters=filters,
		fields=[
			"medical_code",
...
def company_on_trash(doc, method):
	for su in healthconnect.get_all("Healthcare Service Unit", {"company": doc.name}):
		service_unit_doc = healthconnect.get_doc("Healthcare Service Unit", su.get("name"))
