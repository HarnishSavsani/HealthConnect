// Healthcare
healthconnect.ui.form.on('Sales Invoice', {
	refresh(frm) {
	...
	patient(frm) {
		if (frm.doc.patient) {
			healthconnect.db.get_value("Patient", frm.doc.patient, "customer")
				.then(r => {
	...
	if (frm.doc.service_unit && frm.doc.items.length > 0) {
		frm.doc.items.forEach((item) => {
			if (!item.service_unit) {
				healthconnect.model.set_value(item.doctype, item.name, "service_unit", frm.doc.service_unit);
			}
		});
	}
};
...
var make_list_row= function(columns, invoice_healthcare_services, result={}) {
	...
	columns.forEach(function(column) {
		contents += `<div class="list-item__content ellipsis">
			${
				head ? `<span class="ellipsis">${__(healthconnect.model.unscrub(column))}</span>`
...
var set_primary_action= function(frm, dialog, $results, invoice_healthcare_services) {
	...
	dialog.set_primary_action(__('Add'), function() {
		...
		if(checked_values.length > 0){
			...
			if(invoice_healthcare_services){
				healthconnect.msgprint(__("Please select Healthcare Service"));
			}
			else{
				healthconnect.msgprint(__("Please select Drug"));
			}
		}
	});
};
...
var add_to_item_line = function(frm, checked_values, invoice_healthcare_services){
	if(invoice_healthcare_services){
		...
		for(let i=0; i<checked_values.length; i++){
			var si_item = healthconnect.model.add_child(frm.doc, 'Sales Invoice Item', 'items');
			healthconnect.model.set_value(si_item.doctype, si_item.name, 'item_code', checked_values[i]['item']);
			healthconnect.model.set_value(si_item.doctype, si_item.name, 'qty', 1);
			if(checked_values[i]['qty'] > 1){
				healthconnect.model.set_value(si_item.doctype, si_item.name, 'qty', parseFloat(checked_values[i]['qty']));
			}
		}
		frm.refresh_fields();
	}
