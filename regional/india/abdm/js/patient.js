healthconnect.ui.form.on('Patient', {
	refresh: function (frm) {
		if (healthconnect.boot.sysdefaults.country == 'India') {
...
let search_by_abha_address = function (frm) {
	let dialog = new healthconnect.ui.Dialog({
...
let verify_health_id = function (frm, recieved_abha_number = '') {
	let d = new healthconnect.ui.Dialog({
		...
		primary_action(values) {
			...
			healthconnect.run_serially([
				() =>healthconnect.db.get_value('Patient', {abha_number: d.get_value('healthid'), name: ['!=', frm.doc.name]	}, ['name', 'abha_card'])
					.then(r =>{
						if (r.message.name) {
							healthconnect.set_route("Form", "Patient", r.message.name);
...
let create_abha = function (frm) {
	...
	print_button.on('click', 'button', function() {
		healthconnect.db.get_value('Terms and Conditions', d.get_value('patient_consent'), 'terms')
		.then(r => {
			let result = healthconnect.render_template(r.message.terms, {"doc" : {}})
			healthconnect.render_pdf(result, {orientation:"Portrait"});
		})
	})

	healthconnect.db.get_value('ABDM Settings', {
	...
	dialog.fields_dict.healthid.$wrapper.find('.control-input').append(
		`<span class="link-btn" style="display:inline">
			<a class="btn-open no-decoration" title="${__("Scan")}">
				${healthconnect.utils.icon('scan', 'sm')}
			</a>
		</span>`
	);
...
let setup_search_btn = function(dialog) {
	dialog.fields_dict.username.$wrapper.find('.control-input').append(
		...
		<a class="search-icons" title="${__("Search")}">
			${healthconnect.utils.icon("search", "sm")}
			</a>
		</span>`
	);
						...
						healthconnect.call({
							...
							callback: function (data) {
								if (data.message['healthIdNumber']) {
									dialog.hide()
									healthconnect.run_serially([
										() =>healthconnect.db.get_value('Patient', {abha_number: data.message['healthIdNumber'],
...
let verify_mobile_otp_dialog = function(dialog, txn_id, url_key) {
	let otp_dialog = new healthconnect.ui.Dialog({
	...
	healthconnect.run_serially([
		...
		callback: function (data) {
			if (data.message['file_url']) {
				...
				let abha_id_dialog = new healthconnect.ui.Dialog({
					title: 'ABHA Card',
					fields: [
						{
				...
				primary_action(values) {
					...
					healthconnect.render_pdf(result, {orientation:"Landscape"});
				},
				})
