...
def get_authorization_token():
	client_id, client_secret, auth_base_url = healthconnect.db.get_value(
		"ABDM Settings",
		{"company": healthconnect.defaults.get_user_default("Company"), "default": 1},
	...
	req = healthconnect.new_doc("ABDM Request")
	...
	try:
		...
		healthconnect.log_error(message=traceback, title="Cant create session")
...
def abdm_request(payload, url_key, req_type, rec_headers=None, to_be_enc=None, patient_name=None):
	...
	base_url = healthconnect.db.get_value(
		"ABDM Settings",
		{"company": healthconnect.defaults.get_user_default("Company"), "default": 1},
		[url_type],
	)
	if not base_url:
		healthconnect.throw(title="Not Configured", msg="Base URL not configured in ABDM Settings!")
	...
	try:
		...
		req.insert(ignore_permissions=True)
		traceback = f"Remote URL {url}\nPayload: {payload}\nTraceback: {e}"
		healthconnect.log_error(message=traceback, title="Cant complete API call")
		return response.json()
...
def set_consent_attachment_details(doc, method=None):
	if healthconnect.db.exists(
		...
		if doc.consent_for_aadhaar_use:
			file_name = healthconnect.db.get_value("File", {"file_url": doc.consent_for_aadhaar_use}, "name")
			if file_name:
		...
		if doc.abha_card:
			abha_file_name = healthconnect.db.get_value(
